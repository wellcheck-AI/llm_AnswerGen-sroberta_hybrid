...
tok = AutoTokenizer.from_pretrained(
    config["embedding_model"]["model_path"], clean_up_tokenization_spaces=True
)

vectorizer = pk.load(
    open(
        os.path.join(os.path.dirname(__file__), "config", "params", "tfidf_params.pkl"),
        "rb",
    )
)
...
class Document_:
    ...
    def _sentence_embedding(self, query: str) -> List[float]:
        ...
        mean_pooling_embedding = torch.sum(embeddings * mask, 1) / torch.clamp(
            mask.sum(1), min=1e-9
        )
    ...
    def find_match(self, query):
        ...
        if sparse_vector["indices"]:
            ...
            result = index.query(vector=embed_query, top_k=10, include_metadata=True)
